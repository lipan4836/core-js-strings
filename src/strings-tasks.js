...
function removeFirstOccurrences(str, value) {
  ...
  return idx !== -1 ? str.slice(0, idx) + str.slice(idx + value.length) : str;
}
...
function countVowels(str) {
  return !str.replace(/\s/g, '').match(/[aeiouy]/gi)
    ? 0
    : str.replace(/\s/g, '').match(/[aeiouy]/gi).length;
}
...
function isPalindrome(str) {
  return (
    ...
    str.replace(/\W/g, '').split('').reverse().join('').toLowerCase()
  );
}
...
function reverseWords(str) {
  return str
    .split(' ')
    .map((substr) => substr.split('').reverse().join(''))
    .join(' ');
}
...
function encodeToRot13(str) {
  const alhab = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz!? ';
  const rot13alphab = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm!? ';

  return str
    .split('')
    .map((el) => {
      const position = alhab.indexOf(el);
      return rot13alphab[position];
    })
    .join('');
}
